...
function render_videos(projectId) {
    var moduleId = document.querySelector(".main-wrap-aulas .select-cursos").value
    console.log("moduleId", moduleId);
    var videos = get_data(projectId).filter(v => v.id_ondemand_modulo == moduleId)
    videos = videos.map(x => {
        x.trial = 0
        return x
    })
    ...
    console.log("h1", videos)
    document.querySelector(".wrap-cards-videos").innerHTML = ejs.render(home_sales_videos, { videos: videos, project_id: projectId });
}

function init_func_new(projectId){

    render_videos(projectId)
    $('.tabs-content-c .tabs').tabs({
     swipeable:true
    });
    $('.professors-wrap .carousel').carousel({
        numVisible: 3,
        shift: -90,
        indicators: true,
        dist: -50
    ...
    // $(".select-cursos").formSelect();
    // console.log("reee", $(".browser-default.select-cursos"));

    $(document).ready(function(){
    $('.tabs-content-c select').formSelect();
      $('.collapsible').collapsible({
        onOpenStart: function(e) {
            // console.log("e", e),
            // $(e).find('.collapsible-header').css({
            //     'box-shadow': '0px -5px 30px -5px #d3d3d3',
            //     'border-bottom': 'none'
            //  });
            $(e).find('.collapsible-header i').removeClass('fa-plus').addClass('fa-minus')
        },
        onCloseEnd: function(e) {
            // $(e).find('.collapsible-header').css({
            //     'box-shadow': '0px 0px 30px #d3d3d3'
            //  });
            $(e).find('.collapsible-header i').removeClass('fa-minus').addClass('fa-plus')
        }
    });
  });
    // Dinamic calculation of tabs height
    var imgSales = document.querySelector(".sales-background");
    var maxHtmlHeight = $( window ).height() + 300;
    var requiredTopHeight = $(".curso-header").outerHeight() + 200;
    var tabsHeight = maxHtmlHeight - requiredTopHeight;
    document.querySelector(".tabs-content-c .tabs-content").style.height = tabsHeight + "px";
    document.querySelector(".tabs-content-c .scrolling-wrapper")
    ...
    console.log("imgSales", imgSales);
    $(window).scroll(function (event) {
        var offset = $( document ).height() - $( window ).height();
        var transparency = $(window).scrollTop() / offset;
        console.log("transparency ", transparency)
        $(".sales-background-img").css({
...
function get_module_select_options(projectId) {
    var selectOptions = []
    console.log("pid", projectId);
    videos = get_data(projectId)
    videos.forEach(video => { 
        if (!selectOptions.find(o => o.module_id == video.id_ondemand_modulo)) {
            selectOptions.push({module_name: video.modulo, module_id: video.id_ondemand_modulo})
        }
    })
...
function open_home_sale(ele){
    ...
    dados = {}
    dados.id_projeto = id_projeto
    let user = get_data('geral');
    busca_aulas_ondemand({
        id_contato: user ? user.id : 1,
        id_projeto: id_projeto,
        id_modulo: null
    }, (stst) => {
        videos = get_data(id_projeto)
        console.log("id_projeto, videos", id_projeto, videos)
        moduleSelectOptions = get_module_select_options(id_projeto)
        console.log("moduleSelectOptions",moduleSelectOptions);
        ...
        if(tipo == 1){
            get_info_aula_sale(dados, function(gias){
                ...
                change_view(home_sales_v_new, true, true, {
                    ...
                    module_select_options: moduleSelectOptions
                }, navbar_barra_roxa_sales, { titulo: 'Titulo' })
                init_func_new(id_projeto);
                require_loader('close')
            })
        }else{
            get_info_aula_sale(dados, function(gias){
                ...
                // $('body').css({
                //     'background-image': 'url("image/foto_professores_liveu.png")',
                //      'background-size': 'contain',
                //      'background-repeat': 'no-repeat'
                //  })
                $('body').addClass("sales-background");
                require_loader('close')
            })
